...
app.get('/send-review-emails', async (req, res) => {
    var linehtml ='' ;
    var email = '';
...
async function fetchProductDetails(products) {
  try {
    ...
    products.forEach(async(product,index) => {
       ...
       const response = await axios.get(`${productApiUrl}/${product.product_id}.json`,{headers});
...
async function fetchOrderDetails() {
  try {
    const response = await axios.get(orderApiEndpoint);
