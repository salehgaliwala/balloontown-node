...
app.get('/send-review-emails', async (req, res) => {
    ...
    const orderDetails = await fetchOrderDetails();
     ...
     // const productDetails = await fetchProductDetails(obj.line_items);
        // Define email body
        var email = '';
        const timestamp = new Date(orderItem.deliveryTimestamp);
        if (timestamp <= today) {
...
//  console.log(moment().format('YYYY-MM-DD HH:mm:ss'));
       const response = await axios.get(
          `https://cdn.shopify.com/s/files/1/0708/4412/1361/t/34/assets/settings.json?${Date.now()}`
         ).then(async (data) => {
          const jsonData = data.data;
